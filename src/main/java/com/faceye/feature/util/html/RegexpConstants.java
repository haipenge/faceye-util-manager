...
public class RegexpConstants {
	...
	/**
	 * 提取A标签里面的URL <a[^>]*?href=[\"\\']?([^\"\\'>]+?)[\"\\']?[^>]*>.+?<[\\s]*?\\/a> <a[^>]*?href=[\"\\']?([^\"\\'>]+)[\"\\']?[^>]*>.+?<[\\s]*?\\/a>
	 * <a[^>]*?href=[\"\']?([^\"\'>]+?)[\"\']?[^>]*>.+?<[\s]*?\/a>
	 */
	...
	/**
	 * 提取网页的meta数据 <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
	 */
	public static final String DISTILL_HTML_META = "<meta\\s+([^>]*?)=[\"\\']?([^>]*?)[\"\\']?\\s+([^>]*?)=[\"\\']?([^>]*?)[\"\\']?[\\s]*?\\/>";

	/**
	 * 提取IMG <div class="image feature-img thumb-180"> <a href="/p/219266.html">
	 * <img alt="Blank" class="image lazyload" data-src="http://a.36krcnd.com/photo/2015/1ff2b4884bd11d44923fde11c8fc89c8.png!slider" src=
	 * "http://d.36kr.com/assets/images/blank.gif" /> </a> </div> ---------------------------------------
	 ...
	 */
	/**
	 * 提取Img标签里面的data-src <img[^>]*?data\-src=[\"\']?([^\"\'>]+)[\"\']?[^>]*\/>
	 */
	public final static String DISTILL_IMG_DATA_SRC = "<img[^>]*?data\\-src=[\"\\']?([^\"\\'>]+)[\"\\']?[^>]*\\/>";

	/**
	 * 提取Img标签里面的src <img[^>]*?src=[\"\\']?([^\"\\'>]+)[\"\\']?[^>]*\\/> <img[^>]*?src=[\"\']?([^\"\'>]+)[\"\']?[^>]*\/>
	 */
	...
	/**
	 * 匹配INS exam: <ins class="adsbygoogle" style="display:inline-block;width:120px;height:600px" data-ad-client="ca-pub-1090193214637198" data-ad-slot="7813820853"></ins>
	 */
	 ...
	 * 
	 * @author @haipenge haipenge@gmail.com Create Date:2015年1月19日
	 */
	static class URL {
		// 子域名
	...
	/**
	 * 替换掉span <span style="font-family:SimHei; font-size:14px"></span> <[\/\s]*?span[^>]*?>
	 */
	...
	/**
	 * 提取Cnblog博客园子首页 文章明细链接 http://home.cnblogs.com/blog/page/2/
	 */
